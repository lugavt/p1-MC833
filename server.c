...
void* handle_client(void* arg) {
    ...
    //    }
    // }
    printf("Nova conexão com cliente");
}
...
int main() {
    ...
    struct sockaddr_in server_address; //sockaddr_in é uma estrutura que armazena o endereço e a porta
    server_address.sin_family = AF_INET;
    server_address.sin_addr.s_addr = inet_addr(ip);
    server_address.sin_port = port;

    if (bind(server_socket, (struct sockaddr*)&server_address, sizeof(server_address)) < 0) {
        perror("Erro ao vincular o socket do servidor ao endereço");
        exit(1);
 ...
 // até aqui ja temos o servidor configurado
